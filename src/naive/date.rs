...
use crate::format::{
    parse, parse_and_remainder, write_hundreds, Formatter, FormattingSpec, Item, Numeric, Pad,
    ParseError, ParseResult, Parsed, StrftimeItems,
};
#[cfg(any(feature = "alloc", feature = "std"))]
use crate::format::{DelayedFormat, BAD_FORMAT};
use crate::{expect, try_opt};
use crate::{Datelike, IsoWeek, Months, NaiveDateTime, NaiveTime, Utc, Weekday};
...
impl NaiveDate {
    ...
    pub fn format_to_string(&self, fmt_str: &str) -> Result<String, ParseError> {
        let formatter =
            Formatter::<_, Utc>::new(Some(*self), None, None, StrftimeItems::new(fmt_str));

...
const MDL_TO_OL: &[i8; MAX_MDL as usize + 1] = &[
    ...
    34, 36, 94, 96, 94, 96, 94, 96, 94, 96, 94, 96, 94, 96, 94, 96, 94, 96, 94, 96, 94, 96, 94, 96,
    ...
    36, 38, 98, 100, 98, 100, 98, 100, 98, 100, 98, 100, 98, 100, 98, 100, 98, 100, 98, 100, 98,
...
impl Mdf {
    ...
    /// Makes a new `Mdf` value from month and `YearFlags`, setting the day to the last of the
    /// month.
    ///
    /// This method doesn't fully validate the range of the `month` parameters, only as much as what
    /// can't be deferred until later. The year `flags` are trusted to be correct.
    ///
    /// # Errors
    ///
    /// Returns `None` if `month > 12`.
    #[inline]
    pub(super) const fn from_last_day_of_month(
        month: u32,
        YearFlags(flags): YearFlags,
    ) -> Option<Mdf> {
        match month >= 1 && month <= 12 {
            true => Some(Mdf((month << 9) | flags as u32)),
            false => None,
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_mdf_from_last_day_of_month() {
        // regular year
        let year_flags = YearFlags::from_year(2023);
        let mut ordinal = 0;
        for (month0, days) in [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31].iter().enumerate() {
            ordinal += days;
            let mdf = Mdf::from_last_day_of_month(month0 as u32 + 1, year_flags).unwrap();
            assert_eq!(ordinal, mdf.ordinal().unwrap());
        }

        // leap year
        let year_flags = YearFlags::from_year(2024);
        let mut ordinal = 0;
        for (month0, days) in [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31].iter().enumerate() {
            ordinal += days;
            let mdf = Mdf::from_last_day_of_month(month0 as u32 + 1, year_flags).unwrap();
            assert_eq!(ordinal, mdf.ordinal().unwrap());
        }
